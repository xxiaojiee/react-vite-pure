...
const defaultFooterDom = (
  <DefaultFooter copyright={`${new Date().getFullYear()} Ant Design`} links={[]} />
);
...
const BasicLayout = (props: BasicLayoutProps) => {
  ...
  return (
    <ProLayout
      ...
      rightContentRender={() => (
        <RightContent theme={defaultSettings.navTheme} layout={defaultSettings.layout} />
      )}
      {...props}
      route={props.route}
      {...defaultSettings}
    >
      <Authorized
        ...
        currentAuthority={user?.character || ''}
        noMatch={noMatch}
      >
        {children}
      </Authorized>
