...
const transform: AxiosTransform = {
  ...
  beforeRequestHook: (config, options) => {
    const { apiUrl, joinPrefix, joinParamsToUrl, formatDate, joinTime = true, urlPrefix: newUrlPrefix } = options;

    if (joinPrefix) {
      config.url = `${newUrlPrefix}${config.url}`;
    }
  ...
  responseInterceptorsCatch: (error: any) => {
    ...
    try {
      ...
      throw new Error(errs as unknown as string);
    }
